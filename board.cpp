...
public:
    FigureManager(){}
    FigureManager(int type){_type=type;}
...
private:
    int _type;//0:stop  1:circle  2:line  3:rec
};

class Circle:public FigureManager
{
public:
    virtual void input(int x,int y,int r);
...
void Circle::input(int x,int y,int r)
{
    _x=x;
    _y=y;
    _r=r;
...
{
    out<<"Circle{center_X: "<<_x<<", center_Y: "<<_y<<", radius: "<<_r<<"}"<<ends;
}

class Line:public FigureManager//2
{
public:
    virtual void input(int x1,int y1,int x2,int y2);
    virtual void display(BlackBoard &board);
    virtual void print(ostream &out);
...
{
    out<<"Line{point1_X: "<<_x1<<", point1_Y: "<<_y1<<", point2_X: "<<_x2<<", point2_Y"<<_y2<<"}"<<ends;
}

class Rec:public Line//3
{
public:
    virtual void input(int left,int top,int right,int bottom);
    virtual void display(BlackBoard &board);
    virtual void print(ostream &out);
...
void Rec::input(int left, int top, int right, int bottom)
{
    _left=left;
    _top=top;
    _right=right;
...
void Rec::display(BlackBoard &board)
{
...
void Rec::print(ostream &out)
{
    out<<"Rectangle{left: "<<_left<<", top: "<<_top<<",right: "<<_right<<", bottom:"<<_bottom<<"}"<<ends;
}

void FigureManager::input(istream &is)
{
    int in;//一次输入的一个int
    int cnt=0;
    ...
    switch (_type)
    {
        ...
        {
            Circle circle;
            circle.input(data[1], data[2], data[3]);
        ...
        {
            Line line;
            line.input(data[1], data[2], data[3], data[4]);
            line.display(board);
            ostrstream out;
            line.print(out);
            cout<<out.str()<<endl;
        ...
        {
            Rec rec;
            rec.input(data[1], data[2], data[3], data[4]);
            rec.display(board);
            ostrstream out;
            rec.print(out);
            out<<out.str()<<endl;
